...
fold = StratifiedKFold(n_splits=2, shuffle=True, random_state=62)
...
def write_to_csv():
    ...
    k=0
    file_dir = "byte_files"
    files = os.listdir("data/"+file_dir)
    files.sort()
    train_labels=pd.read_csv("data/trainlabels.csv")
    train_np = np.zeros((len(files),len(a.split(","))))
    
    for file_ in tqdm(files):
        statinfo = os.stat("data/"+file_dir+"/"+file_)
        with open("data/"+file_dir+"/"+file_,"r") as fp:  
